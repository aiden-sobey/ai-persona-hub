...
export async function modelCommand(): Promise<void> {
  ...
  const model = await inquirer.prompt([{
    ...
    default: currentModel && PROVIDER_MODELS[provider.value as AIProvider].includes(currentModel) 
      ? currentModel 
...
export async function modelListCommand(): Promise<void> {
  ...
  if (currentProvider && currentModel) {
    console.log(chalk.green(`✓ Current: ${currentProvider} - ${currentModel}\n`));
  } else {
    console.log(chalk.yellow('⚠️  No model configured. Use "cgem model" to set one.\n'));
  }
